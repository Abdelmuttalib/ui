...
export function PageHeaderActions({ className, ...props }: { className?: string; children: React.ReactNode }) {
  return <div className={cn('flex flex-col gap-2 sm:flex-row sm:items-center', className)} {...props} />
}

export function PageTitle({ ...props }: React.ComponentProps<typeof Typography>) {
  return <Typography as="h1" variant="3xl/semibold" className="mb-2 tracking-tight" {...props} />
}

export function PageDescription({ ...props }: React.ComponentProps<typeof Typography>) {
...
export function FormActions({ className, ...props }: { className?: string; children: React.ReactNode }) {
  return (
    <Inline
      className={cn('mt-6 flex flex-col-reverse items-stretch justify-end sm:flex-row', className)}
      gap="compact"
      {...props}
    />
  )
...
export function FormLayout({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('grid grid-cols-1 gap-x-10 gap-y-6 md:grid-cols-2 xl:grid-cols-3', className)} {...props} />
}
...
export function FormSectionHeader({ icon, title }: { icon?: React.ReactNode; title: string }) {
  return (
    ...
    >
      {icon && (
        <div className="bg-primary/15 flex h-8 w-8 items-center justify-center rounded-lg text-primary [&>svg]:h-5 [&>svg]:w-5">
          {icon}
        </div>
      )}
...
export function Grid({ gap = 'section', className, ...props }: GridProps & DivPropsType) {
  return <div className={cn('grid grid-cols-12', gridGaps[gap], className)} {...props} />
}
...
export function GridItem({ col = 'col-span-12', className, ...props }: GridItemProps & DivPropsType) {
...
export function Inline({ gap = 'compact', className, ...props }: InlineProps) {
  return <div className={cn('flex items-center', gridGaps[gap], className)} {...props} />
}
